...
- 인공신경망이란 무엇인가? 
	- 생물의 뉴런 연결구조를 모방하여 만든 네트워크 구조이다. 
...
- Gradient Vanishing 이 무슨 문제인지 아는가?
	...
	- 이 떄 acitvation function으로 시그모이드 함수 대신 ReLu 계열 함수를 사용하는 것을 하거나, batch normalization을 통해 Gradient Vanishing 문제를 어느정도 완화시킬 수 있습니다
...
- 이진 검색 binary search : $O(\log_{}{n})$
	- 오름차순 또는 내림차순으로 정렬된 배열이 있을 때, 배열의 median 원소값과 target 원소값을 비교하여, 원하는 키값이 더 큰경우 median 을 포함한 왼쪽 원소들을 탐색 영역에서 제거, 원하는 키값이 더 작은 경우 median을 포함한 오른쪽 원소들을 제거하는 방법
- 해시법 hash search
...
### 재귀 알고리즘
- 알고리즘이 자기 자신을 사용하여 정의되는 경우를 해당 알고리즘을 재귀 알고리즘 recursive algorithm 이라 정의한다
- 예
	- 팩토리얼
	- 유클리드 호제법: 최대 공약수 구하기
	- 하노이의 탑
...
- 버블 정렬- 단순 교환 정렬
	...
	- 개선안
		- 버블 알고리즘 개선 버전 1
			- 어떤 한번의 패스 과정동안 원소 교환이 이루어지지 않는다면, 모든 원소가 정렬된 상태이기 때문에 정렬하는 것은 멈추는 조건문을 추가한다
		- 버블 알고리즘 개선버전 2
...
- 단순 선택 정렬 straight selection sort 
	- 시간 복잡도
		- $O(n^2)$
	- 가장 작은 원소부터 선택하여 알맞은 위치 원소와 위치를 교환하는 것을 반복하여, 정렬하는 알고리즘
- 단순 삽입 정렬 straight insertion sort 
	...
	- 두번째 원소부터 target 원소가 된다. target 원소 앞의 원소 배열들과 크기를 비교하여, 원소 배열이 계속 오름차순일수 있도록, target 원소를 삽입한다.
- 셸 정렬 shell sort
	- 시간복잡도
		- Best: $O(n)$
		- Avg: $O(n^{1.5})$
		- Worst: $O(n^2)$
	- 정렬할 배열의 원소들을 그룹으로 나누어, 각 그룹별로 정렬을 수행, 정렬된 그룹을 반복하여 원소들의 이동횟수를 줄이는 방법이다
	- 단순 삽입 정렬의 장점(이미 정렬을 마쳤거나 정렬이 거의 마쳐가는 상황에서 속도가 빠름)을 갖고, 단점(삽입할 위치가 멀리 떨어져 있다면 이동속도가 많아짐)을 보완한다
	- 배열이 $[x_1,x_2,...,x_n]$ 이 있다 할 경우 $gap_1=n//2$ 의 간격으로 원소들 간 크기를 비교하여 정렬한다. $gap_k=n//2^k$의 간격으로 원소들간 크기들을 비교하여 정렬한다
- 퀵 정렬 quick sort
	- 시간복잡도
		- Best: $O(n\cdot \log_{2}{n})$
		- Avg: $O(n\cdot \log_{2}{n})$
		- Worst: $O(n^2)$
	- 피봇 Pivot이란 분할의 기준이 되는 원소를 정해 계속해서 배열을 이분할 하는 방법이다
	- 시간복잡도가 $O(n\cdot \log_{}{n})$으로 많은 상황에서 가장 빠른 알고리즘으로 알려져 있다
	- 그러나 매번 1개와 나머지 원소로 그룹이 나눠지는 경우 시간 복잡도는 $O(n^2)가 될 수 있다
	- 서로 이웃하지 않는 원소를 교환하므로 불안정한 알고리즘이다
	- 배열을 두 그룹으로 나누기
...
- 병합 정렬 merge sort : $O(n\cdot \log_{}{n})$ 이다
	...
	- 하나의 배열을 두개의 배열로 쪼개고, 분할된 배열을 정렬한 다음, 분할된 두 배열을 다시 합쳐 하나의 정렬된 배열로 만드는 알고리즘이다
	- 두 배열을 하나로 합칠 때는 리스트값을 하나씩 비교해가며, 두 리스트값중 더 작은 값을 새로운 리스트로 옮긴다
- 힙 정렬 heap sort:  $O(n\cdot \log_{2}{n})$
	- 시간복잡도
		- Best : $n\cdot \log_{2}{n}$
		- Avg: $n\cdot \log_{2}{n}$
		- Worst: $n\cdot \log_{2}{n}$
	- 힙(heap: 부모의 값이 자식의 값보다 항상 크다는 조건을 만족시키는 완전 이진 트리)의 특성을 활용하여 정렬하는 알고리즘
	- 만약 힙에 새로운 요소가 들어온다면, 일단 마지막 노드에 삽입을 한다. 그리고 부모 노드와의 크기를 비교하면서 원소를 교환한다
	- 힙 정렬은 전체 자료를 정렬하는 것이 아니라 가장 큰 몇개 값만 정렬하므로, 시간복잡도에서 좋은 성능을 보여주는 편이다
...
#### 리스트의 종류
- 연결 리스트 linked list
	- 노드가 1차원적으로 연결되어 있고, $1 \to 2\to 3\to ... \to N$로 한쪽 방향으로 호출할 수 있지만, 건너 뛰거나 반대방향으로 호출할 수 없는 구조
...
- 관련 용어들
    ...
    - 노드의 가족관계
        - 어떤 노드 $y$ 가 가지로 연결된 아래쪽 노드 $x_{1},x_{2},...,x_{n}$을 가지고 있다면
            ...
            - $y$에서 아래쪽 방향으로만 가지를 타고 이동하였을 때 연결되어 있는 노드를 $y$의 자손$descendant$라 부른다
